...
def fetch_usage(base_url: str, api_key: str):
    ...
    try:
        payload = json.loads(resilient_fetch_url(url, user_agent="codexzh-api-usage-skill/1.0").decode("utf-8"))
        return 200, payload
    except Exception as fallback_error:
        raise SystemExit(
            "Network error while querying CodexZH: "
            f"{describe_network_error(last_error)}; proxy fallback failed: {fallback_error}"
        )
